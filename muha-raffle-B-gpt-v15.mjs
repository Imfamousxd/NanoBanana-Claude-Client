...
const PROMPT = `MUHA MEMBERS GIVEAWAY RAFFLE CARD — VARIATION B (TAILGATE — CAST IN THE TRUCK BED).
...
The ONLY differences from Variation A: (1) cast is posed in/around a black Ford F-150 XLT pickup truck with the tailgate down, (2) a subtitle "WIN A FORD F-150 XLT" is added in cream beneath "GIVEAWAY".
...
TRUCK — IMPORTANT: a BLACK FORD F-150 XLT pickup truck from the truck reference (image 2) is part of the scene. The truck is parked with the TAILGATE DOWN, three-quarter rear view (you can see the open tailgate, the bed, the rear lights, the rear bumper). The truck is rendered in PIXAR-3D animated style matching the character world (NOT photoreal). The cast is posed IN the truck bed and around the open tailgate — some sitting on the tailgate edge, some standing in the bed, some leaning against the side. The truck is the supporting hero element behind/around the cast. Strip all real-world branding from the truck BODY itself (no Ford emblem text on the tailgate, no dealership signage, no license plate text — the truck body is clean blackout). The truck must read as a recognizable Ford F-150 XLT pickup. Position the truck and characters so they all stay INSIDE the inner border (border still clips the topmost layer — nothing crosses the gold frame).
...
form.append("image[]", new Blob([truckBuf], { type: "image/jpeg" }), "truck.jpg");
...
console.log("Generating Raffle B v15 (template-matched border)...");
...
const outPath = `${outDir}/${stamp}_muha-raffle-B-tailgate-v15.png`;
