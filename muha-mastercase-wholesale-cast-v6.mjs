...
Exactly these 10, each once, zero duplicates. Vibrant, premium, colorful, full-bleed. The only text/marks are: the Members logo (from ref 12), "GIVEAWAY", "WIN A FORD MUSTANG", "MASTER CASE", "ALL-IN-ONE". No QR, no other text. Spell everything exactly.`;
...
const outs=[];
data.forEach((item,i)=>{if(!item?.b64_json)return;const out=`AI Fruit VIdeos Muha/Master Case Designs/${stamp}_mastercase-wholesale-cast-v6-c${i+1}.png`;fs.writeFileSync(out,Buffer.from(item.b64_json,"base64"));console.log("✓ "+out);outs.push(out);});
console.log("\nDone — "+outs.length);
