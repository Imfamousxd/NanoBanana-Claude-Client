...
| `dam_catalog` | The reviewed product catalog for a brand: SKU lines with state-prefixed names, generation, codes, flavours, render counts (current design vs previous), and the render groups that are categories (devices, group shots, badges). `dam_product_refs`, `job_create` and `context_pack` resolve products against it first; fuzzy search only when a name is unknown. Built by `npm run content -- skus catalog --brand muha --library <render-library.json>` from `knowledge/skus/` (registry + folder map + folder rules). |
