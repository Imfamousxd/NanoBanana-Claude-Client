#!/usr/bin/env node
// Move 10mg batch outputs from the output dir into
// Noble Harbor Wholesale/<Peptide>/10mg/<stem>_10mg_<size>_<color>.jpg
// Matches each job to its output by the slug tag at the start of the prompt (parallel-worker safe).
// Usage: node nh-rename-10mg.mjs <batch.json>

import fs from "fs";
import path from "path";

const batchFile = process.argv[2];
if (!batchFile) { console.error("Usage: node nh-rename-10mg.mjs <batch.json>"); process.exit(1); }

const jobs = JSON.parse(fs.readFileSync(batchFile, "utf8"));
const GENS = process.env.OUTPUT_DIR || "nh-outputs-10mg";
...
for (const job of jobs) {
  ...
  const folder = path.join("Noble Harbor Wholesale", meta.peptide, "10mg");
  fs.mkdirSync(folder, { recursive: true });
  const ext = path.extname(match);
  const dst = path.join(folder, `${stemOf(meta.peptide)}_10mg_${meta.size}_${meta.color}${ext}`);
