...
// ─── API call ─────────────────────────────────────────────────────────────────
async function generateImage({ prompt, aspectRatio, imageSize, refImages, background, n = 1 }) {
  if (!API_KEY) throw new Error("OPENAI_API_KEY not set in .env");
  n = Math.max(1, Math.min(10, n)); // gpt-image-2 accepts n up to 10 per call
  ...
  if (useEdits) {
    ...
    const payload = { model: MODEL, prompt, size, quality, n };
...
// ─── Save results ─────────────────────────────────────────────────────────────
// `dest` is optional. With a batch `_id`, candidates land at <dir>/c1.png … cN.png;
// without one, the legacy timestamped flat naming is used unchanged.
...
async function runBatch(batchFile) {
  ...
  console.log(`\n=== OpenAI Batch Mode: ${jobs.length} job(s), ${totalCandidates} candidate(s) ===\n`);
  ...
  for (let i = 0; i < jobs.length; i++) {
    ...
    try {
      // Unlike Gemini, gpt-image-2 takes n natively — one call returns n candidates.
      ...
      const dest = id ? { dir: path.join(OUTPUT_DIR, batchName, id), base: "c" } : null;
      const produced = saveResults(response, job.prompt, dest);

      // Stable hero path for the frame->clip handoff (see nanobanana.mjs for the why).
