...
- EXACTLY TWO ARMS, EXACTLY TWO HANDS, five fingers each — TWO arms only, no extra limbs.
- Pose: playful gamer energy — e.g. throwing a cute PEACE SIGN / V near her eye, OR holding a glowing game CONTROLLER, OR one hand on the headset ear-cup like she's mid-stream saying "let's go". Lively, animated, fun body language. NOT crying, NOT pained.
...
for (const part of data?.candidates?.[0]?.content?.parts || []) {
  if (part.inlineData) {
    const outPath = `generations/${new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19)}_muha-fruit-blue-slushie-v2-egirl-NB.png`;
