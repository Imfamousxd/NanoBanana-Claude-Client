...
- **People vs objects: `seedance-2.0` CATEGORICALLY REFUSES photoreal human first frames** (E005 'flagged as sensitive'). Use **`bytedance/seedance-1.5-pro` for any talking head or person**; 2.0 for objects/scenes only. Settled by controlled experiment 2026-07-28 (`sieve/golden/experiments/E1-e005-whitelist/`): a Seedream-4 portrait and a Nano Banana portrait fail identically, the same portrait still fails with every person-word stripped from the prompt, and a non-human frame with that same neutral prompt succeeds. So the trigger is the IMAGE, not the phrasing — **unlike the E005-on-phrasing case below, rephrasing does NOT fix this**. The "route human frames through Seedream to inherit ByteDance trust" idea is dead; do not retry it.
- **`resolution` on 1.5-pro is UNDECLARED BUT HONOURED — always send `1080p`.** It is absent from the model's OpenAPI schema, yet Replicate passes it through and the model uses it. Measured A/B 2026-07-28 (`sieve/golden/experiments/E4-avatar-through-seedance/`): identical 1536×2752 first frame + identical prompt → **1080×1920 with it, 720×1280 without**. A declared schema is not the contract; never gate this field on schema presence. Output does NOT inherit the first frame's resolution.
- **Consequence:** the quality lever for people-video is BOTH — send 1080p, and spend on the frame, because the frame carries identity/wardrobe/set and a bad one guarantees a bad clip at 10× the price. Gate the frame with `sieve-judge` before animating.
