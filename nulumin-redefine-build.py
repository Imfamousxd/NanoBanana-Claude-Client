#!/usr/bin/env python3
# Build batch JSONs for the corrected-theme 72-shot regen (12 per theme), locked grit-film recipe.
import json, os
...
GRADE = {"r03": "warm golden, heavily directional-blurred, near-abstract", "r04": "dark teal-and-olive, moody, glowing halation",
         "r06": "soft, muted grey-toned interior, candid and unposed", "r09": "near-abstract heavy directional smear with ghosting",
         "r10": "muted earthy dawn-outdoor tones, naturalistic", "r12": "muted, soft, candid daylight"}
...
EXTREME_BLUR = ("THIS IS A LONG-EXPOSURE MOTION SHOT WHERE MOTION IS THE SUBJECT. The moving person is almost entirely DISSOLVED into long "
                ...
                "exactly like the reference. The WHOLE frame is swept with heavy motion. Do NOT keep the subject sharp — it must be aggressively, "
                "obviously, heavily blurred and smeared. The blur is by far the dominant feature.")
FROZEN = ("Frozen sharp at a fast shutter, catching the person in the very middle of the moment — caught in the act, candid and real, NOT posed.")
CODE = {"weight-loss": "wl", "recovery": "rec", "longevity": "lon", "performance": "perf", "cognitive": "cog", "sexual-health": "sex"}

concepts = json.load(open("nulumin-redefine-concepts.json"))
jobs = []
for theme, items in concepts.items():
    code = CODE[theme]
    for i, c in enumerate(items[:12], 1):
        tag = f"nul-{code}-{i:02d}"
        motion = EXTREME_BLUR if c["mode"] == "blur" else FROZEN
        prompt = (f"{tag}: A raw, grainy 35mm film photograph. {c['scene']} Subject: {c['subject']}. {motion}\n\n"
                  f"{FILM}\n\nMatch the {GRADE[c['ref']]} film look of the reference image, but render THIS scene.")
        jobs.append({"prompt": prompt, "aspectRatio": c["aspect"], "imageSize": "4K", "refImages": [REF[c["ref"]]]})
N = 8
batches = [jobs[i::N] for i in range(N)]
for i, b in enumerate(batches, 1):
    json.dump(b, open(f"nulumin-redef-p{i}.json", "w"), indent=2)
print(f"{len(jobs)} jobs across {N} batches:", [len(b) for b in batches])
