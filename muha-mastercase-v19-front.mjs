...
Portrait near-square panel. Top to bottom, keep every zone in the SAME place/size as image 0:
  ...
  3. Two winged flame crests flanking the headline top — left [0.26,0.23,0.13,0.09], right [0.61,0.23,0.13,0.09]. Keep as cool decorative winged crests (re-skin to neon-gold). Must sit fully inside the guard border, below the top line.
...
async function genOne(i) {
  ...
  const outPath = `${outDir}/${stamp}_muha-mastercase-v19c${i}-FRONT.png`;
  fs.writeFileSync(outPath, Buffer.from(item.b64_json, "base64"));
  console.log(`✓ cand ${i}: ${outPath}`);
  return outPath;
}

console.log(`Generating ${N} v19 FRONT candidates (prompt-only re-roll; pick cleanest top)...`);
