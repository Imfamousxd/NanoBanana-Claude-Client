...
Replace image 0's lone truck with a HERO ENSEMBLE: the FORD F-150 XLT (glossy blackout pickup, from the truck reference — preserve its real shape/details) as the PRIZE, with the 10 Muha fruit-drama characters gathered around and in front of it as the heroes. Characters = heroes, truck = trophy behind them. All 10 distinct Pixar-3D fruit-headed figures at consistent scale. Exactly ONE blueberry-blue character; no duplicates, no extra figures.
CONTAINMENT — IMPORTANT: the ENTIRE ensemble (truck + all 10 characters, including every limb, hand, leaf, hair, prop, and shadow) must sit FULLY INSIDE the central hero zone [0.13,0.49,0.74,0.19] with margin. SCALE THE ENSEMBLE DOWN as needed so it fits — it must NOT touch or cross the side guard-borders, must NOT rise into the GIVEAWAY ribbon above (stay below y=0.49), and must NOT drop onto the MM coin / ALL-IN-ONE / checkered band below (stop above y=0.68, leaving a clear empty gap before the coin). Leave a clear gap between the characters and every surrounding zone and the blue guardrail on all sides. Characters should read as a contained group portrait within the frame, not a full-bleed crowd.
...
console.log("Generating Master Case v15 FRONT (top + bottom guardrails reinforced)...");
...
const outPath = `${outDir}/${stamp}_muha-mastercase-v15-FRONT.png`;
