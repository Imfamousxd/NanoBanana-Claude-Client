...
Portrait near-square panel. Top to bottom, keep every zone in the SAME place/size as image 0:
  ...
  2. TOP EMBLEM ROW [0.20,0.10,0.60,0.11] — see EMBLEM SWAP #1.
...
=== EMBLEM SWAP #1 — TOP (the only top emblem) ===
Replace image 0's top "Muha Meds + Von Dutch Kulture House" dual lockup with the MUHA MEMBERS logo ALONE, single and centered in the top emblem row. Composite the Muha Members logo reference AS-IS, pixel-faithful: blue circular verification-checkmark badge + gold "M" monogram + gold cursive "Members®". Do NOT redraw, recolor, neon-ify, or add any text to it. There is NO Von Dutch and NO Muha Meds winged-M up top. (The logo already contains its own wordmark — do NOT add a separate "MUHA MEMBERS"/"MUHA" text line.)
...
=== TEXT (verbatim) ===
"FORD" / "F-150 XLT" (headline) · "GIVEAWAY" · "ALL-IN-ONE" · "100 UNITS | 10 CASES | 10 STRAINS" · inverted top-flap tagline (small) · 21+ fine print if present. The top Muha Members logo and bottom MM coin are composited IMAGES, not typed text — do NOT type "MUHA MEMBERS", "MUHA", or "Von Dutch" anywhere. Spell everything exactly.

NEGATIVE: no layout change / no zone moved or resized / no crop / no aspect change; no content crossing fold lines; no character or truck over the headline/ribbon/coin; no black chrome; no Von-Dutch style or text; no Muha Meds winged-M up top; no garbled or redrawn logos (composite the logo + coin refs as-is); no duplicate characters; no extra brands; no phone numbers/URLs/barcodes/dates; no misspellings.`;
...
// image 0 = front ground truth, then logo, coin, truck, 10 chars
...
console.log("Generating Master Case v12 FRONT (split front ref ground truth, Members logo + MM coin)...");
...
const outPath = `${outDir}/${stamp}_muha-mastercase-v12-FRONT.png`;
