...
CRITICAL COMPOSITION RULE — the BADGE SIGN MUST BE FULLY VISIBLE AND UNOBSTRUCTED in the UPPER portion of the frame, clearly readable edge to edge. Frame the character WIDER (pulled back, NOT a tight close-up) — a WAIST-UP / hip-up shot showing her whole upper body, both full arms, both hands, and her full outfit, with some breathing room of background around her on the sides. She is a bit smaller in the frame than a close-up so more of her and the room is visible. Keep a comfortable air gap between the top of her head and the bottom of the badge.
...
- Adult feminine proportions, cute and lively. Framed WAIST-UP / hip-up (wider shot, not a close-up) so her full upper body and outfit are visible.
...
COMPOSITION: 9:16 vertical, poster-style layout. Upper portion = badge sign fully visible edge to edge. Character framed WAIST-UP / hip-up (wider, pulled-back shot — NOT a tight close-up), full upper body and both arms visible, with background breathing room around her.
...
for (const part of data?.candidates?.[0]?.content?.parts || []) {
  if (part.inlineData) {
    const outPath = `generations/${new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19)}_muha-fruit-blue-slushie-v6-egirl-NB.png`;
