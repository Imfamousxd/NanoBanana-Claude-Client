...
VERTICAL LAYOUT:
- VERY TOP: leave a CLEAN, UNCLUTTERED horizontal band across the top with a simple, slightly darker/calm backdrop (NO text, NO wordmark, NO characters, NO busy fruit there) — this empty space is RESERVED for a logo to be placed later. Do NOT draw any logo or brand name.
- Below that reserved band: the headline — bold "GIVEAWAY" in vivid BLUE, with "WIN A FORD MUSTANG" in clean WHITE beneath. Spell both exactly.
- CENTER: a sleek BLACK FORD MUSTANG sports car (reference image 11) on a glossy lit stage, low cinematic three-quarter angle, with the 10 characters as a balanced cast lineup flanking and standing behind it.
- BOTTOM (centered): a lockup reading "MASTER CASE" on the first line and "ALL-IN-ONE" on the second line (premium gold tagline, centered). NO QR code anywhere. NO barcode. Nothing in the corners.
...
Exactly these 10, each once, zero duplicates. Vibrant, premium, colorful. The ONLY text is "GIVEAWAY", "WIN A FORD MUSTANG", "MASTER CASE", "ALL-IN-ONE". No QR, no other text. Spell everything exactly.`;
...
const outs=[];
data.forEach((item,i)=>{if(!item?.b64_json)return;const out=`AI Fruit VIdeos Muha/Master Case Designs/${stamp}_mastercase-wholesale-cast-v3-c${i+1}.png`;fs.writeFileSync(out,Buffer.from(item.b64_json,"base64"));console.log("✓ "+out);outs.push(out);});
console.log("\nDone — "+outs.length);
