...
VERTICAL LAYOUT with clean breathing room reserved top and bottom:
- VERY TOP (centered, small): the "Members" brand wordmark — the word "Members" with a small CHECKMARK badge/tick beside it. Keep it small and clean at the top.
- Just below it: the headline — bold "GIVEAWAY" in vivid BLUE, with "WIN A FORD MUSTANG" in clean WHITE beneath. Spell both exactly.
...
- BOTTOM: leave a clean lower band. Centered in it, a lockup reading "MASTER CASE" on the first line and "ALL-IN-ONE" on the second line (premium, like a product tagline). At the BOTTOM-RIGHT corner, a QR CODE (a clean square QR placed with room around it).
...
Exactly these 10, each once, zero duplicates. Vibrant, premium, colorful. The only text is: "Members", "GIVEAWAY", "WIN A FORD MUSTANG", "MASTER CASE", "ALL-IN-ONE". Spell everything exactly.`;
...
const outs=[];
data.forEach((item,i)=>{if(!item?.b64_json)return;const out=`AI Fruit VIdeos Muha/Master Case Designs/${stamp}_mastercase-wholesale-cast-v2-c${i+1}.png`;fs.writeFileSync(out,Buffer.from(item.b64_json,"base64"));console.log("✓ "+out);outs.push(out);});
console.log("\nDone — "+outs.length);
