...
const PROMPT = `Reference 1 is the "BLUE SLUSHIE" flavor badge. It MUST appear in the scene AS-IS — a FLAT RECTANGULAR PRINTED POSTER pinned to the wall behind the character, reproducing reference 1 EXACTLY and pixel-faithfully. The badge is an emblem with TWO stacked banners: the word "BLUE" in chunky ICY-BLUE frosted/cracked-ice BLOCK capital letters with a dark outline on the upper banner, and the word "SLUSHIE" in BUBBLEGUM-PINK rounded balloon capital letters with a dark outline on the lower pink banner, with several chunky translucent ICE CUBES clustered framing the banners. Reproduce that exact emblem — same two-banner layout, same blue block "BLUE", same pink balloon "SLUSHIE", same ice cubes, same colors and outlines — as a flat printed poster. It is NOT a neon sign, do NOT make it glow, do NOT turn it into neon tubing, do NOT redraw, restyle, recolor, re-letter, or reinterpret it in any way. Reference 1 placed AS-IS on a rectangular poster, pixel-faithful, unchanged. The badge poster is the only source of brand/typography in the scene.

CRITICAL COMPOSITION RULE — the BADGE SIGN MUST BE FULLY VISIBLE AND UNOBSTRUCTED in the UPPER portion of the frame, clearly readable edge to edge. Frame the character as a WIDE KNEE-UP / three-quarter shot (pulled WAY back, NOT a close-up) — show her from roughly the KNEES UP, so her full torso, hips, both full arms, both hands, the full top, and the upper part of her legs/bottoms are all visible. She is noticeably smaller in the frame so MUCH MORE of her body and the room shows. Keep a comfortable air gap between the top of her head and the bottom of the badge.
...
- Adult feminine proportions, cute and lively. Framed KNEE-UP (wide three-quarter shot, NOT a close-up) so her full torso, hips, the full outfit and the tops of her legs are all visible.
...
OUTFIT — cute gamer e-girl streetwear, NO HOODIE, PLAIN solid colors, NO graphics/prints/logos/text:
- ABSOLUTELY NO HOODIE and no hood. Instead, a cute fitted e-girl TOP — for example a cropped tee / fitted baby tee, a cami/tank top, or a cute cropped zip crop-jacket — in icy blue / bubblegum pink two-tone (color-blocked). Solid colors only, NO prints, NO graphics, NO logos, NO text, NO embroidery, NO hood.
- Layer it with cute e-girl touches: striped arm warmers or fingerless gloves in the blue/pink palette, and a small choker. Optional cute layered long-sleeve under a tank. Keep it cute and vibrant.
- BOTTOMS (now visible in the knee-up shot): cute e-girl bottoms in the blue/pink palette — for example a pleated mini skirt with thigh-high striped socks, or comfy cuffed sweatpants/joggers, or shorts — solid colors, NO prints/graphics/logos. Tasteful and cute (young adult).
- Modern fashionable e-girl / gamer aesthetic, comfy and colorful — but NOT a hoodie.

WALL & BADGE PLACEMENT & BACKGROUND — a cozy GAMER BEDROOM / streaming setup wall washed in blue + pink RGB LED light (the e-girl RGB aesthetic, which doubles as the cold slushie palette). The "BLUE SLUSHIE" badge from reference 1 is a FLAT RECTANGULAR PRINTED POSTER on that wall, occupying the upper portion, large and completely visible edge to edge — reproduced EXACTLY and pixel-faithfully from reference 1 (two banners, blue block "BLUE", pink balloon "SLUSHIE", ice cubes), NOT a neon sign, NOT glowing, NOT restyled. Soft blurred hints of a gaming setup (LED strips, soft bokeh) in the background, shallow depth of field.
...
COMPOSITION: 9:16 vertical, poster-style layout. Upper portion = badge poster fully visible edge to edge. Character framed KNEE-UP (wide three-quarter, pulled-WAY-back shot — NOT a close-up), full torso + hips + tops of legs + both arms visible, with generous background breathing room around her.

STYLE LOCK — hyper-detailed Pixar / Cinema 4D / Octane 3D render, animated-feature-film look, unified Pixar-3D stylization across the ENTIRE character (slushie head, face, hands, headset, top). NOT photoreal, NOT live-action. Hands are PIXAR-CARTOON hands. Clothes and headset are PIXAR-CARTOON (stylized, animated feature shading). One cohesive Pixar 3D animated character matching the other approved characters in the set.

NEGATIVE — do NOT render her crying / sad / teary / in brain-freeze pain (she is vibrant and happy now). Do NOT use a human-skin face — the whole head/face/hands must be translucent blue+pink slushie. Do NOT omit the cat-ear gaming headset. Do NOT omit the pink slushie-ice hair. Do NOT use any human flesh tone anywhere. Do NOT add ANY graphics, prints, brand logos, or text to the clothing. Do NOT redraw, restyle, recolor, re-letter, or turn the badge into a neon sign — the badge poster must match reference 1 EXACTLY. Do NOT put her in a hoodie or any hood. Do NOT add a microphone or mic boom to the headset. Do NOT put a game controller or any object in her hands. Do NOT add a third hand or third arm — exactly two hands total, no extra limbs. Do NOT redraw the badge poster — reference 1 placed AS-IS. Do NOT add other characters. Do NOT add the Muha brand logo.`;
...
for (const part of data?.candidates?.[0]?.content?.parts || []) {
  if (part.inlineData) {
    const outPath = `generations/${new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19)}_muha-fruit-blue-slushie-v7-egirl-NB.png`;
