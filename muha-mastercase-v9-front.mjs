...
const PROMPT = `FORD F-150 XLT GIVEAWAY — Muha master-case FRONT PANEL. This is a LAYOUT-LOCKED RESTYLE / EDIT of the supplied primary image (image 0), NOT a redesign and NOT a new layout. Image 0 is the absolute STRUCTURAL GROUND TRUTH. You must reproduce its dieline, its zone geometry, its element positions, its element sizes, its alignments, and its text — ALL IDENTICAL to image 0. The ONLY two things you are permitted to change are: (1) the visual STYLE (re-render the existing black / chrome / Von-Dutch KUSTOM pinstripe look as Pixar/C4D Miami-Vice neon), and (2) the INTERIOR CONTENT of the single central hero zone (swap the lone truck for the 10-fruit-character-plus-truck ensemble). Nothing else may move, resize, rotate, reshape, duplicate, merge, appear, or disappear.
...
- USE THESE NORMALIZED COORDINATES (origin = top-left, values are fractions of the full canvas width/height as [x, y, w, h]) AS HARD POSITIONS. Each element must land in its box, at its size. Do not nudge:
  ...
  4. LEFT emblem lozenge = MUHA MEMBERS logo — [0.18, 0.11, 0.26, 0.10]. Turquoise/blue checkmark "M" badge above gold "Members" + "MUHA MEMBERS" lettering.
  5. RIGHT emblem lozenge = winged-M crest (mirror composition) — [0.56, 0.11, 0.26, 0.10]. Gold winged-M monogram crest with "MUHA" beneath.
  ...
  9. Gold winged-M crest MEDALLION — [0.38, 0.66, 0.24, 0.10]. Circular medallion + symmetric wings, centered, sits BELOW the central hero zone. Untouched in position and size.
...
- "MUHA MEMBERS" (left emblem) and "MUHA" (right crest)
- "ALL-IN-ONE"
- "100 UNITS | 10 CASES | 10 STRAINS" (pipe separators)
- 21+ fine print line (small)
- Inverted tagline on the top dust flap (small)
No other text. No Von Dutch wordmark anywhere. Spell everything exactly as written.
...
console.log("Generating Master Case v9 FRONT (layout-locked restyle/edit of v5, coordinate-pinned)...");
...
const outPath = `${outDir}/${stamp}_muha-mastercase-v9-FRONT.png`;
