...
Negative: do not use slab serif on headline, do NOT make headline lines different sizes, do not include any raffle ticket or card, do not include any QR code, do not warp the MM logo, do not show any faces, no extra logos, do not use the word GIVEAWAY or WIN or WINNER anywhere, do NOT show the car at an angle (FRONTAL HEAD-ON ONLY), do not add bright background details — background must be nearly black with subtle red tint only.`;
...
const jobs = [
  {
    ...
    _meta: { name: "muha-members-only-v2" },
  },
];

fs.writeFileSync("muha-members-only-v2.json", JSON.stringify(jobs, null, 2));
