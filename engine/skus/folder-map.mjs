...
// For a product folder that feeds no SKU line: which book lines could it be? Same state, same category, no
// strength/cannabinoid contradiction, and evidence: flavours of the line found in the folder's file names or
// vision reads, size and strength words in common. Lines with uncovered flavours rank first.
const CATEGORY_OF_FOLDER = [[/^Renders\/(Hemp\/)?Disposables/i, "disposables"], [/^Renders\/(Hemp\/)?Cartridges/i, "cartridges"], [/^Renders\/(Hemp\/)?Pre[- ]?Rolls/i, "pre-rolls"], [/^Renders\/(Hemp\/)?Edibles/i, "edibles"], [/^Renders\/(Hemp\/)?Concentrates/i, "concentrates"], [/^Renders\/(Hemp\/)?Flower/i, "flower"], [/^Renders\/Pod/i, "pods"], [/^Renders\/(Moods|Mavricks|MMxCookies)/i, "disposables"], [/^Renders\/Hemp\/Pre Rolls\/Hash Holes/i, "concentrates"]];
const STRENGTH_TOKENS = [["live resin", /live ?resin|\blr\b/], ["melted diamond", /melted ?diamond|\bmd\b/], ["hash rosin", /hash ?rosin|\bhr\b/], ["distillate", /distillate|distallite/], ["thca", /thc-?a\b|thca/], ["thcp", /thc-?p\b|thcp/], ["hhc", /\bhhc\b/], ["d8", /\bd8\b|delta[- ]?8/], ["d10", /\bd10\b|delta[- ]?10/], ["d9", /\bd9\b|delta[- ]?9/], ["piatella", /piatella/], ["bubble hash", /bubble ?hash/], ["kief", /\bkief/], ["sift", /\bsift/], ["temple ball", /temple ?ball/], ["caviar", /caviar/]];
function suggestCandidates(folders, registry, coverage) {
  const uncoveredByLine = new Map(coverage.lines.map((l) => [l.id, new Set(l.itemsDetail.filter((i) => !i.renders).map((i) => i.name.toLowerCase()))]));
  const marketOf = (p) => (p.match(/(?:^|\/)(CA|MI|MO|NJ|NM|NY|OH|AZ)(?:\/|_| |$)/) || [])[1] || (/^Renders\/Hemp\//i.test(p) ? "HEMP" : null);
  for (const f of folders) {
    f.candidates = [];
    if (f.lines.length || f.status || !f.words) continue;
    const category = (CATEGORY_OF_FOLDER.find(([re]) => re.test(f.path)) || [])[1]; if (!category) continue;
    const market = f.market || marketOf(f.path);
    const folderStrength = STRENGTH_TOKENS.filter(([, re]) => re.test(f.words)).map(([k]) => k);
    const scored = [];
    for (const line of registry.lines) {
      if (line.category !== category) continue;
      if (market && line.market && line.market !== market) continue;
      const lineText = ` ${(line.name || line.line).toLowerCase()} `;
      const lineStrength = STRENGTH_TOKENS.filter(([, re]) => re.test(lineText)).map(([k]) => k);
      if (lineStrength.length && folderStrength.length && !lineStrength.some((k) => folderStrength.includes(k))) continue;
      const uncovered = uncoveredByLine.get(line.id) || new Set();
      const found = line.items.map((i) => i.name.toLowerCase().replace(/\s*\(.*?\)\s*/g, " ").trim()).filter((n) => n.length >= 4 && f.words.includes(" " + n.replace(/[^a-z0-9.]+/g, " ") + " "));
      const foundUncovered = found.filter((n) => [...uncovered].some((u) => u.replace(/\s*\(.*?\)\s*/g, " ").trim() === n));
      let score = found.length + foundUncovered.length * 2;
      const why = [];
      if (found.length) why.push(`${found.length} of its ${line.items.length} flavours are in this folder (${found.slice(0, 4).join(", ")}${found.length > 4 ? "…" : ""})`);
      const sharedStrength = lineStrength.filter((k) => folderStrength.includes(k)); if (sharedStrength.length) { score += 2; why.push(`same kind: ${sharedStrength.join(", ")}`); }
      if (line.format && f.words.includes(` ${line.format.toLowerCase()} `)) { score += 1; why.push(`same size ${line.format}`); }
      if (uncovered.size === line.items.length && line.items.length) { score += 1; why.push("this line has no renders at all yet"); }
      if (score >= 2) scored.push({ id: line.id, name: line.name || line.line, skuBlock: line.skuBlock || null, score, why: why.join(" · ") });
    }
    f.candidates = scored.sort((a, b) => b.score - a.score).slice(0, 3);
  }
}
export async function buildFolderMap(root, { brand = "muha", libraryFile, page, decisions }) {
  ...
  const rulesFile = path.join(root, "knowledge", "skus", `folder-rules.${brand}.json`);
  const rules = fs.existsSync(rulesFile) ? JSON.parse(fs.readFileSync(rulesFile, "utf8")).folders || {} : {};
  ...
  for (const g of groups.values()) {
    ...
    g.words = " " + [...new Set(g.assets.flatMap((a) => [a.path.split("/").pop().replace(/\.[a-z0-9]+$/i, ""), a.product || "", a.title || ""]))].join(" ").replace(/_/g, " ").replace(/([a-z])([A-Z])/g, "$1 $2").toLowerCase().replace(/[^a-z0-9.]+/g, " ") + " ";
    delete g.assets;
  }
  ...
  const folders = [...groups.values()].map((g) => {
    ...
    const folder = { id: g.id, path: g.path, files: g.files, assigned: g.assigned, market: top(g.markets), status, compositions: Object.fromEntries(g.compositions), lines, samples: g.samples, outliers: g.outliers || [], checked: g.checked, unassigned: g.unassigned || [], words: g.words || "", decision: known.get(g.path) || null };
    ...
    const rule = rules[g.path] || null;
    if (rule) {
      folder.rule = rule;
      if (rule.tag) folder.designTag = gen !== null ? `Gen ${gen} · ${rule.tag}` : rule.tag;
      if (rule.kind) folder.kind = rule.kind;
      if (rule.outliersOk) { folder.outliersOkCount = folder.outliers.length; folder.outliers = []; }
    }
    folder.proposal = proposalOf(folder);
    folder.meaning = meaningOf(folder);
    return folder;
  }).sort((a, b) => a.path.localeCompare(b.path));
  suggestCandidates(folders, registry, coverage);
  ...
  const map = { brand, builtAt: new Date().toISOString(), scope: brand === "muha" ? "Renders/ only" : "all", naming, folders: folders.map(({ samples, outliers, words, ...rest }) => ({ ...rest, samples: samples.map((s) => ({ id: s.id, path: s.path })), outliers: outliers.map((o) => ({ id: o.id, path: o.path, why: o.why })) })) };
...
export async function buildReviewPage(root, map, registry) {
  ...
  for (const [parent, folders] of [...parents.entries()].sort((a, b) => a[0].localeCompare(b[0]))) {
    ...
    for (const f of folders) {
      ...
      const kind = f.kind ? `<div class="kind${f.lines.length ? " small" : ""}">${esc(f.kind)}</div>` : "";
      const plan = f.rule && (f.rule.moveTo || f.rule.copyTo || f.rule.copyNote) ? `<p class="plan">Dropbox plan: ${esc(typeof f.rule.moveTo === "string" ? "move to " + f.rule.moveTo : f.rule.moveTo ? Object.entries(f.rule.moveTo).map(([g, t]) => `move ${g} → ${t}`).join("; ") : "")}${f.rule.copyTo ? " copy to " + esc(f.rule.copyTo) : ""}${f.rule.copyNote ? " " + esc(f.rule.copyNote) : ""}</p>` : "";
      const okNote = f.outliersOkCount ? `<div class="check ok">Checked all ${f.checked.files} files: ${f.outliersOkCount} looked different and a reviewer confirmed they belong.</div>` : "";
      ...
      const cands = (f.candidates || []).length ? `<div class="cands"><b>Likely match${f.candidates.length > 1 ? "es" : ""} in the book:</b>${f.candidates.map((c) => `<div class="cand"><button data-assign="${c.id}" data-label="${esc(c.name)}${c.skuBlock ? " · " + esc(c.skuBlock) : ""}">Assign to ${esc(c.name)}${c.skuBlock ? " · " + esc(c.skuBlock) : ""}</button><span class="dim small">${esc(c.why)}</span></div>`).join("")}</div>` : "";
...
document.addEventListener("click",async(ev)=>{const b=ev.target.closest("button");if(!b)return;const row=b.closest("article.row");if(b.hasAttribute("data-assign")&&row){await save(row.dataset.id,{verdict:"line",lineId:b.dataset.assign,lineLabel:b.dataset.label});return;}if(b.hasAttribute("data-group-ok")){const sec=b.closest("section.group");for(const id of sec.dataset.ids.split(" ")){if(!state[id])await save(id,{verdict:"ok"});}return;}if(!row)return;const id=row.dataset.id;const v=b.dataset.v;if(v==="line"){const f=row.querySelector(".lineform");f.hidden=!f.hidden;if(!f.hidden)fillSelect(row);return;}await save(id,{verdict:v,lineId:null,lineLabel:null});});
