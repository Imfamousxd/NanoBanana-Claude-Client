...
export class DamWorker {
  ...
  async probeRemote(asset, source) {
    ...
    if (!exact) await this.db.enqueue("analyze", { assetId: asset.id, sourceId: source.id, priority: asset.kind === "image" ? 70 : 80 });
  ...
  async probe(assetId) {
    ...
    try {
      ...
      if (!exact) await this.db.enqueue("analyze", { assetId, sourceId: source.id, priority: asset.kind === "image" ? 70 : 80 });
  ...
  // ---------- analyze (paid) ----------
  async guardSpend(estimateUsdNext) {
    if (!this.config.approved) throw new SpendNotApproved("Paid analysis is locked. Set DAM_APPROVED=1 (or pass --approve) after confirming scope and cost.");
  ...
  async analyze(assetId) {
    const asset = await this.db.getAsset(assetId);
    const source = await this.db.getSource(asset.source_id);
    await this.guardSpend(estimateUsd(this.config.vision.model, asset.kind === "video" ? "vision-video" : "vision-image"));
    ...
    try {
      ...
      await this.db.enqueue("embed", { assetId, sourceId: source.id, priority: 90 });
      return { ok: true };
    } finally {
      cleanup();
    }
  ...
  async analyzeLight(asset, source) {
    ...
    await this.db.enqueue("embed", { assetId: asset.id, sourceId: source.id, priority: 90 });
    return { ok: true, light: true };
  }

  // ---------- embed (paid, cheap) ----------
  async embedAsset(assetId) {
    const asset = await this.db.getAsset(assetId);
    if (!asset.search_doc) throw new Error("asset has no search document yet");
    await this.guardSpend(estimateUsd(this.config.embed.model, "embed", 2));
  ...
  async runJob(job) {
    switch (job.kind) {
      ...
      case "analyze": return this.analyze(job.asset_id);
      case "embed": return this.embedAsset(job.asset_id);
