...
export function createDamTools(root, graph) {
  ...
  return [
    ...
    {
      name: "dam_catalog",
      title: "Reviewed product catalog (SKU lines → flavours → renders)",
      description: "The catalog a person reviewed folder by folder: every SKU line with its state-prefixed name ('CA 1G Distillate Disposables'), generation, codes, flavours and how many renders each flavour has (current design vs previous), plus the render groups that are a category rather than a SKU (devices, group shots, badges). Use these names in dam_product_refs / job_create. Free; reads a JSON file.",
      inputSchema: { brand: brandField, line: z.string().optional().describe("Filter: part of a line name, e.g. 'MI 2G Distillate'"), flavour: z.string().optional().describe("Filter: part of a flavour name") },
      handler: async ({ brand, line, flavour }) => { const { loadCatalog } = await import("../skus/catalog.mjs"); const b = String(brandOf(brand) || "muha").replace(/^brand\./, "").split("-")[0]; const catalog = loadCatalog(root, b); if (!catalog) return { note: `No reviewed catalog for ${b}. Build it with: npm run content -- skus catalog --brand ${b} --library <render-library.json>` }; const l = line ? line.toLowerCase() : null; const f = flavour ? flavour.toLowerCase() : null; const lines = catalog.lines.filter((x) => !l || x.name.toLowerCase().includes(l)).map((x) => ({ id: x.id, name: x.name, market: x.market, category: x.category, generation: x.generation, skuBlock: x.skuBlock, folders: x.folders.map((y) => `${y.path.replace(/^Renders\//, "")}${y.designTag ? " [" + y.designTag + "]" : ""}`), flavours: x.items.filter((i) => !f || i.name.toLowerCase().includes(f)).map((i) => ({ name: i.name, sku: i.sku, renders: i.assets.length, current: i.assets.filter((a) => a.current).length, compositions: [...new Set(i.assets.map((a) => a.compositionLabel))] })) })).filter((x) => !f || x.flavours.length); return { brand: b, builtAt: catalog.builtAt, totals: catalog.totals, lines, groups: line || flavour ? undefined : catalog.groups.map((g) => ({ path: g.path, kind: g.kind, designTag: g.designTag, files: g.files })) }; },
    },
    {
