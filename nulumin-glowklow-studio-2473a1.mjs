#!/usr/bin/env node
// Glow + Klow studio shots — CORRECTED deeper blue to match the Epithalon example.
// Target accent sampled from the approved Epithalon streak shot's "10mg" dose ink + cap:
//   #3d8fc0 = RGB(36,115,161) — a medium cerulean / steel-blue (NOT the old light #94c7e2).
// Step 1: render Glow studio ANCHOR off the CFC ref with the cap+accent pushed to #3d8fc0.
...
const BLUE = "a vivid cerulean blue, base hue around hex #3d8fc0 = RGB(61,143,192). MATERIAL — this is a smooth molded OPAQUE PLASTIC cap (like a colored plastic bottle cap), NOT metal. Finish is SATIN / semi-gloss plastic: a soft, broad, diffuse sheen where the studio light gently rolls across the curve, brightening to a soft light cerulean (around #6db4dd) on the lit side and easing to the deeper #3d8fc0 cerulean on the shaded side — a gentle, evenly saturated gradient. CRITICAL — it must NOT look metallic: NO mirror/chrome reflection, NO sharp bright specular hotspot, NO brushed-aluminum streaking, NO steel-grey tint. The plastic reads as a solid, richly saturated, BRIGHT cerulean with a soft matte-to-satin glow, not a flat dark cap and not a shiny metal cap. Do NOT let it go navy/teal, and do NOT let it go pale washed-out baby-blue.";
...
1. CAP COLOR & MATERIAL: ${BLUE}. Same cap shape and height as reference 1's cap, but render it as the molded satin PLASTIC described above (do not copy any metallic look from reference 1's cap) — change the color/material to this cerulean plastic.
2. ACCENT COLOR: the exact same blue #3d8fc0 as the cap, used for the thin vertical accent stripe on the left edge of the label, the divider line under the wordmark, and the dose text. Replace ALL the pink accents in reference 1 with this exact blue.
...
4. LABEL DOSE: replace "5mg" with "70mg" in the same heavy sans italic style, rendered in the accent blue (#3d8fc0).

Render at high resolution, 5:4 aspect ratio matching reference 1.

Negative: do not change the studio background tone or brighten it, do not turn the backdrop pure white, do not add a glossy mirror floor reflection that is not in reference 1, do not change the shadow softness, do not change the vial proportions, do not change the NuLumin Bio-Sciences brand lockup, do not warp the wordmark, do not change the side spec text block, do not change the camera angle, do not add extra graphics or icons, do not change the cap shape (only its color to #3d8fc0), do not let the blue drift light/pale/washed-out or navy/teal/grey, do not put the product name in the accent color (BLACK only).`;

const anchor = await callModel(
  [inline(CFC_REF), { text: ANCHOR_PROMPT }],
  "Glow Blend 70mg studio ANCHOR (#3d8fc0)",
  "NuL_GlowBlend_70mg_studio_anchor_3d8fc0"
);
...
await callModel(
  ...
  "NuL_KlowBlend_80mg_studio_derived_3d8fc0"
);

console.log("\nDone — Glow anchor + Klow derived at corrected #3d8fc0.");
