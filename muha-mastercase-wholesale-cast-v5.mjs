...
const PROMPT=`Create a PREMIUM portrait poster (2:3) for the Muha Members "AI Fruit" Ford Mustang giveaway — vibrant glossy Pixar/Cinema-4D 3D, a lush colorful FRUIT-DRAMA world background.

FULL-BLEED composition: the artwork fills the ENTIRE poster edge to edge. Do NOT add any empty band, blank strip, or letterbox, and do NOT shrink or push the composition down — it should feel full and immersive.

- Keep a modest amount of the fruit-world SKY/background as CONTINUOUS headroom across the top ~15 percent (real lush fruit/sky background art — NOT an empty, flat, or dark band, NOT a letterbox; do not place anything there). The bold "GIVEAWAY" headline (vivid BLUE) with "WIN A FORD MUSTANG" (clean WHITE) beneath it sits JUST BELOW that headroom. Do NOT shrink the characters or the car to make this room — keep the cast and Mustang LARGE and filling the frame; only the headline is positioned a bit lower.
- CENTER: a sleek BLACK FORD MUSTANG sports car (reference image 11) on a glossy lit stage, low cinematic three-quarter angle, with the 10 characters as a balanced cast lineup flanking and standing behind it, filling the frame.
- BOTTOM (centered): a lockup reading "MASTER CASE" on the first line and "ALL-IN-ONE" on the second line (premium gold tagline, centered). NO QR code, NO barcode, nothing in the corners.
...
Exactly these 10, each once, zero duplicates. Vibrant, premium, colorful, full-bleed. The ONLY text is "GIVEAWAY", "WIN A FORD MUSTANG", "MASTER CASE", "ALL-IN-ONE". No QR, no logo, no other text. Spell everything exactly.`;
...
const outs=[];
data.forEach((item,i)=>{if(!item?.b64_json)return;const out=`AI Fruit VIdeos Muha/Master Case Designs/${stamp}_mastercase-wholesale-cast-v5-c${i+1}.png`;fs.writeFileSync(out,Buffer.from(item.b64_json,"base64"));console.log("✓ "+out);outs.push(out);});
console.log("\nDone — "+outs.length);
