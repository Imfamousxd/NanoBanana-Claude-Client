...
Portrait near-square panel. Top to bottom, keep every zone in the SAME place/size as image 0:
  ...
  3. Two winged flame crests flanking the headline top — left [0.26,0.23,0.13,0.09], right [0.61,0.23,0.13,0.09]. Keep as cool decorative winged crests (re-skin to frosted icy-blue + gold). Must sit fully inside the guard border, below the top line.
  4. STACKED HEADLINE "FORD" / "F-150 XLT" [0.12,0.26,0.76,0.13] — two lines, the dominant frosted-chrome + icy-blue 3D lettering (frost-rimmed, cold sheen), ornamental divider above & below. Keep it WITHIN this box with margin; do NOT let it grow upward toward the top guard line or the logo box.
...
=== STYLE CHANGE (whole panel, geometry untouched) — FRUIT-ICE THEME ===
Re-render the ENTIRE panel as unified glossy PIXAR / CINEMA 4D / OCTANE 3D in a cool FRUIT-ICE / FROZEN aesthetic (NOT the previous Miami-Vice retro-neon look — drop the purple-magenta sunset, palm silhouettes, and neon city grid entirely). New mood: a frosty, icy, refreshing freezer-fresh world. Palette: icy ARCTIC BLUES + crisp FROST WHITE + cool TEAL/CYAN, with the vivid fruit colors (red, yellow, green, purple, pink) POPPING against the cold backdrop. Background inside the panel: a chilled gradient of pale icy blue to deeper frozen teal, scattered ice crystals / snowflakes, frosty condensation and cold mist/vapor, sparkling frost bokeh, and dynamic splashes of fresh fruit juice mixed with crushed ice and water droplets. Surfaces look frozen — rimmed with frost and tiny ice crystals; subtle icicles hanging from the headline and ledges. Re-skin all the SAME ornament shapes (headline, crests, ribbon, coin, divider, checkered band, borders) as frosted chrome + icy-blue + glistening gold with a cold sheen and frost edges. The F-150 looks freshly chilled (a light frost sheen, cold rim-light, faint vapor) but stays glossy black and clearly readable. Bright, premium, cold-and-fresh — like a frozen fruit-ice popsicle ad. All text crisp and legible.
...
async function genOne(i) {
  ...
  const outPath = `${outDir}/${stamp}_muha-mastercase-v20c${i}-FRONT.png`;
  fs.writeFileSync(outPath, Buffer.from(item.b64_json, "base64"));
  console.log(`✓ cand ${i}: ${outPath}`);
  return outPath;
}

console.log(`Generating ${N} v20 FRONT candidates (fruit-ice restyle of c3; pick cleanest)...`);
