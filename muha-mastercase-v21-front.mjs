...
const FRONT_REF = "AI Fruit VIdeos Muha/Master Case Designs/2026-05-31T04-49-45_muha-mastercase-v19c3-FRONT.png"; // image 0 = c3 (good layout/top); restyle to fruit-ice
...
TOP GUARDRAIL (THE RECURRING PROBLEM — fix it precisely): the TOP edge of the blue guard-border must be ONE SOLID, CONTINUOUS, PERFECTLY STRAIGHT horizontal blue line spanning the full width of the panel, unbroken end to end, drawn cleanly in front of everything at roughly y=0.085. NOTHING may break, interrupt, overlap, sit on, or cross this top line: not the corner filigree flourishes, not the two winged-eyeball emblems, not the headline, not any glow or ornament. Structure the top of the panel as three stacked bands separated by clear gaps: (a) the angled TOP DUST-FLAP with the small inverted tagline lives ABOVE the top blue line; (b) the top blue guard line itself (solid, continuous); (c) BELOW the line, with margin, comes the blank logo box, then the winged-eyeball emblems + corner flourishes, then the "FORD F-150 XLT" headline. Keep the corner flourishes and emblems INSIDE the guard border (below the top line and inside the side lines) — they must not poke through any guard line. Headline at a moderate size with clear breathing room; do NOT let the headline, crests, or flourishes grow up into or over the top guardrail or into the dust-flap.
...
Portrait near-square panel. Top to bottom, keep every zone in the SAME place/size as image 0:
  ...
  2. TOP EMBLEM ROW [0.22,0.12,0.56,0.10] — see TOP LOGO ZONE (left intentionally BLANK). Sits BELOW the top blue guard line with a clear gap.
  3. Two winged flame crests flanking the headline top — left [0.26,0.23,0.13,0.09], right [0.61,0.23,0.13,0.09]. Keep as cool decorative winged crests (re-skin to glossy candy-color + gold, subtle frost edge). Must sit fully inside the guard border, below the top line.
  4. STACKED HEADLINE "FORD" / "F-150 XLT" [0.12,0.26,0.76,0.13] — two lines, the dominant glossy candy-colored + gold 3D lettering with a juicy sheen and only a subtle frosted edge, ornamental divider above & below. Keep it WITHIN this box with margin; do NOT let it grow upward toward the top guard line or the logo box.
...
=== STYLE CHANGE (whole panel, geometry untouched) — COLORFUL FRUIT (HINT OF FROST) ===
Re-render the ENTIRE panel as unified glossy PIXAR / CINEMA 4D / OCTANE 3D in a VIBRANT, COLORFUL FRESH-FRUIT aesthetic (NOT the Miami-Vice retro-neon look, and NOT a frozen icy-blue world — this should NOT read as "ice cold" or be dominated by arctic blue). Lead with JUICY, SATURATED FRUIT COLOR: a lush tropical fruit-explosion energy — bright fresh fruit (strawberries, citrus/orange & lemon, watermelon, kiwi/green, grapes/purple, berries, mango), glossy juice splashes, droplets, and a warm sunlit pop. Palette: rich reds, oranges, yellows, limes/greens, pinks, purples — warm and appetizing, high saturation, the whole panel feels fresh and flavorful. Background inside the panel: a colorful gradient (warm fruit tones blending into a fresh teal/green, NOT a cold blue field) with dynamic fruit + juice splashes, scattered fresh fruit pieces, and bright bokeh sparkle. ONLY a HINT OF FROST: a light frosty rim on a few edges, a sprinkle of tiny ice crystals / a couple of water-droplet/condensation touches, and a faint cool mist around the truck — just enough to whisper "chilled / refreshing," used sparingly as an accent and NEVER taking over the colorful fruit mood. Re-skin all the SAME ornament shapes (headline, crests, ribbon, coin, divider, checkered band, borders) as glossy candy-colored + gold with a juicy sheen and only a subtle frosted edge. The F-150 stays glossy black and clearly readable with a fresh sheen. Bright, premium, mouth-watering — like a colorful fresh-fruit flavor ad with a refreshing chilled hint. All text crisp and legible.
...
const N = Number(process.env.N_CANDS || 3);
...
async function genOne(i) {
  ...
  if (!res.ok) { console.error(`cand ${i}: HTTP ${res.status}: ${(await res.text()).slice(0, 300)}`); return null; }
  const data = await res.json();
  const item = (data.data || [])[0];
  if (!item?.b64_json) { console.error(`cand ${i}: no b64_json`); return null; }
  const stamp = new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19);
  const outPath = `${outDir}/${stamp}_muha-mastercase-v21c${i}-FRONT.png`;
  fs.writeFileSync(outPath, Buffer.from(item.b64_json, "base64"));
  console.log(`✓ cand ${i}: ${outPath}`);
  return outPath;
}

console.log(`Generating ${N} v21 FRONT candidates (colorful-fruit + hint-of-frost restyle of c3)...`);
const results = await Promise.all(Array.from({ length: N }, (_, i) => genOne(i + 1)));
console.log("done:", results.filter(Boolean).length, "of", N);
