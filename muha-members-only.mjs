...
const PROMPT = `Instagram square 1:1 promo graphic for Muha Members — exclusivity / VIP energy. Same locked Vice City design language as reference 1.
...
5) SUB-INSTRUCTION at ~35% from top: one line of cream tracked-out caps reading "EXCLUSIVE ACCESS — EXCLUSIVE REWARDS" — small but readable, letterpress texture, warm gold tint.

6) HERO ZONE at ~40-72% from top: A dramatic VIP nightclub entrance scene. Two thick red velvet ropes on gold stanchions frame the left and right foreground, creating a corridor leading the eye inward. Beyond the velvet ropes, the red Dodge Challenger is parked on a red carpet under dramatic spotlights, glowing like a prize on display. The background is a dark, upscale Miami nightlife venue exterior — black marble, subtle gold trim, warm ambient lighting, atmospheric haze and fog drifting through the spotlights. The whole scene screams EXCLUSIVE, VIP, BOTTLE-SERVICE ENERGY. Red and gold light dominates. The velvet ropes make you feel like you're being invited past the line.
...
const jobs = [
  {
    ...
    _meta: { name: "muha-members-only" },
  },
];

fs.writeFileSync("muha-members-only.json", JSON.stringify(jobs, null, 2));
